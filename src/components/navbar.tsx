...
export function Navbar() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Welcome to zomatokarbi.com`,
      });
      setIsLoginOpen(false);
    } catch (error: any) {
  ...
  const handleAuthError = (error: any) => {
    ...
    if (error.code === 'auth/unauthorized-domain') {
      ...
      errorMessage = `The domain "${domain}" is not authorized. Please add it to Firebase Console settings.`;
    }
    toast({
            ...
            {userLoading ? (
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  <Avatar className="w-9 h-9 border-2 border-primary/20 p-0.5">
                    ...
                    <AvatarFallback className="bg-primary/10 text-primary font-black">{user.displayName?.charAt(0) || user.email?.charAt(0) || "U"}</AvatarFallback>
                  </Avatar>
              ...
              <Dialog open={isLoginOpen} onOpenChange={setIsLoginOpen}>
                <DialogTrigger asChild>
                  <Button className="rounded-xl font-bold px-6 bg-primary hover:bg-primary/90 shadow-md transition-all">
                    Login
                  </Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[400px] rounded-3xl p-8">
                  <DialogHeader className="mb-6">
                    ...
                    <DialogDescription className="text-center font-medium">
                      Order your favorite food from Diphu
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <form onSubmit={handleEmailLogin} className="space-y-4">
                    ...
                    <Button 
                      type="submit" 
                      className="w-full h-12 rounded-xl font-black text-lg mt-2 shadow-lg shadow-primary/20"
                  ...
                  <Button 
                    ...
                    className="w-full h-12 rounded-xl border-2 hover:bg-muted/50 font-bold flex items-center justify-center gap-3 transition-all"
                    disabled={isAuthLoading}
                  >
                    <svg className="w-5 h-5" viewBox="0 0 24 24">
                      <path fill="#EA4335" d="M12 5.04c1.64 0 3.12.56 4.28 1.67l3.2-3.2C17.52 1.6 14.96 0 12 0 7.31 0 3.33 2.69 1.39 6.6l3.86 3c.94-2.82 3.56-4.96 6.75-4.96z"/>
                      <path fill="#4285F4" d="M23.49 12.27c0-.8-.07-1.57-.21-2.32H12v4.39h6.44c-.28 1.48-1.12 2.74-2.37 3.58l3.69 2.86c2.16-1.99 3.41-4.92 3.41-8.51z"/>
                      <path fill="#FBBC05" d="M5.25 14.61c-.24-.72-.38-1.49-.38-2.29s.14-1.57.38-2.29l-3.86-3C.51 8.5 0 10.19 0 12c0 1.81.51 3.5 1.39 4.98l3.86-2.99z"/>
                      <path fill="#34A853" d="M12 24c3.24 0 5.96-1.07 7.95-2.91l-3.69-2.86c-1.11.75-2.52 1.19-4.26 1.19-3.19 0-5.81-2.14-6.75-4.96l-3.86 3C3.33 21.31 7.31 24 12 24z"/>
                    </svg>
                    Google
                  </Button>
